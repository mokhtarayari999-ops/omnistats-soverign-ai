...
<style>
    ...
    .info-grid { display: flex; justify-content: center; flex-wrap: wrap; gap: 10px; margin-top: 20px; }
...
if st.button("🔱 إطلاق المحاكاة الشمولية العظمى"):
    with st.spinner('🎯 جاري استنفار المحرك السيادي...'):
        time.sleep(1.5)
        ...
        # ⚡ الإضافات الإبداعية الجديدة
        penalty_prob = "عالية (🔥)" if (h_atk + a_atk) > 15 else "متوسطة (⚖️)"
        pressure_idx = int((h_atk + a_atk) * 0.9) # مؤشر التسونامي الهجومي
        ...
        # حاوية الركنيات والبطاقات
        out += "<div class='info-grid' style='margin-top:15px;'>"
        ...
        # 🚀 إضافة مؤشر التسونامي وركلات الجزاء
        out += "<div class='info-grid' style='margin-top:15px;'>"
        out += f"<div class='info-node' style='border-color:#00ff00;'><p>🌊 ضغط هجومي</p><h3>{pressure_idx}/20</h3></div>"
        out += f"<div class='info-node' style='border-color:#ff4b4b;'><p>⚽ ضربة جزاء</p><h3>{penalty_prob}</h3></div>"
        out += "</div>"

        out += "<div style='background:rgba(212,175,55,0.05); border-radius:15px; padding:15px; margin-top:20px; border:1px dashed #D4AF37;'>"
        out += f"<p style='color:#D4AF37; margin:0; font-weight:bold;'>💡 الرؤية السيادية:</p>"
        out += f"<p style='color:#ccc; margin:0; font-size:0.9rem;'>يتوقع مباراة {'مفتوحة' if pressure_idx > 12 else 'مغلقة تكتيكياً'}. رادار التوقيت يرجح الحسم في الشوط الثاني.</p></div>"
